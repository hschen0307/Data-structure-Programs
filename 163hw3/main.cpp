...
void add_app(table & haha)
{
	...
	if(!haha.main_data(hello))		//insert this object to the linear link list(in the table class, it will copying the object's data before insert)
	{
		cout<<"adding data doesn't work."<<endl;
	}
	for(int i=0;i<count;++i)
	{
		if(!haha.insert(key_list[i]))	//insert based on every keyword of this object(it will creat a pointer node in the table to point the head node 's data of the linear link list every time.
		{
